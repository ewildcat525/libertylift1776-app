...
- The Final Push War Room (`/final-push`): the last day lived rather than refreshed. A clock counting down to the closing bell (midnight local) that escalates the whole page through `steady → closing → final-hour → bell`, the national day count ticking up as reps land, an inline log box with `+25/+50/+100` presets so nobody has to leave to answer a rival, the chase number ("312 more to pass X"), a board that reorders with ▲/▼ movement and a "you just got passed" alert, a live tape of every set landing across the country, and the final-day state battle with a pound-for-pound average. Liveness comes from a Supabase realtime subscription to `pushup_logs` inserts, debounced into an authoritative refetch (ranks and ties are decided in SQL), with a 25s poll as a backstop where realtime cannot connect; the closing bell fires a fireworks moment naming the champion. The page has four phases keyed to `finalPushPhase()`: the eve (countdown plus the biggest single day anyone has posted all month), the live room, August 1 results marked provisional through the grace day, and a hand-off to the Hall of Honor after that. Backed by `final_push_pulse`, `final_push_state_board`, and `final_push_feed` views

...
export default function SignupPage() {
  ...
  // Catch-up pace for mid-challenge signups; set after mount to keep the
  // prerendered HTML in sync with the first client render.
  const [catchUpReps, setCatchUpReps] = useState<number | null>(null)
  ...
  useEffect(() => {
    ...
    if (isChallengeLive(now) && reps !== null && challengeDaysRemaining(now) < 31) {
      setCatchUpReps(reps)
    }
  }, [])
    ...
    <main className="auth-page">
      ...
      <section className="auth-shell">
        <div className="auth-copy">
          ...
          {catchUpReps !== null && (
            <p className="auth-pace-note" role="status">
              Joining mid-challenge? Log <strong>{catchUpReps} a day</strong> from
              today and you still finish all 1,776 by July 31.
            </p>
          )}
        </div>
